...
/*   Updated: 2019/07/17 13:50:23 by cterblan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_zone	*get_zone(size_t size){
	...
	while (h_run){
		if (h_run->type == what_type(size))
		{
			...
			if ((unsigned int)((h_run->data + h_run->size - 1) - (b_run->data + b_run->size - 1)) >= size)
				return (h_run);
		}
		h_run = h_run->next;
	}
...
void	*allocate_block(t_zone **zone, size_t size)
{
	...
	if (zone){
		...
		while (b_run->next){
			if ((b_run->next->data) - (b_run->data + b_run->size -1))
		}
	}
	return(NULL);
}
